...
export const LearningStoryMap: React.FC<WidgetProps> = ({ moduleId }) => {
  ...
  if (moduleId === '3.1') {
    ...
    return <div className="story-board">{top}<div className="story-critic-layout"><PhotoTile /><div className="story-output-card">{panels[value]}</div></div><Feedback>{value === 'reasoning' ? '先说明问题，再提出动作；Reasoning 是后续建议和评分的上下文。' : value === 'suggestions' ? '建议把诊断转换成明确方向，供 Artist 执行。' : '评分会在后续训练中用于比较输入与编辑结果的质量。'}</Feedback></div>;
  }
  ...
  if (moduleId === '5.1') {
    ...
    return <div className="story-board"><div className="story-training-steps">{stages.map((stage) => <article key={stage.id} className="story-training-step"><span className="story-training-icon">{stage.icon}</span><span className="story-training-stage">{stage.stage}</span><strong>{stage.title}</strong><b>{stage.goal}</b><p><em>谁更新：</em>{stage.update}</p><small>{stage.note}</small></article>)}</div></div>;
  }
  ...
  if (moduleId === '8.1') {
    ...
    return <div className="story-board">{top}<div className="story-evidence-panel">{evidence[value]}</div><Feedback>{feedback[value]}</Feedback><div className="story-evidence-banner">📊 阅读原则：每个任务只在自己的评估协议与指标方向内比较；不要跨任务直接比较数值。</div></div>;
  }
  ...
  if (moduleId === '9.1') {
    const notes: Record<string, { title: string; text: string; tone: string }> = {
      ...
      key: { title: '共享奖励让两个模块围绕“修得更好”协同', text: '奖励同时要求遵从建议、光度自然和结构一致，使诊断与编辑在同一目标下共同优化。', tone: 'key' },
    };
